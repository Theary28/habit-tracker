...
A small React and Supabase habit tracker with email auth, protected routing, owner-scoped CRUD, and daily completion logs.

## Local setup

1. Create a Supabase project.
2. Copy `.env.example` to `.env` and add the project URL and publishable anon key.
3. Run all of `supabase/schema.sql` in the Supabase SQL editor. It creates both tables, enables RLS, creates the owner policies, and seeds sample rows for the first registered account.
